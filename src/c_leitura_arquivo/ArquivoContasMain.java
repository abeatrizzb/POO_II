...
public class ArquivoContasMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while((linha = br.readLine())!=null) { //nesse loop irá ler e colocar o que foi lido na variável enquanto não estiver nulo
				String v[] = linha.split(","); //armazena em um array as linhas (devem estar separadas por uma ",")
				...
				if(v[0].equals("0")) { //se na posição 0 listar "0" adiciona uma nova conta corrente adicionando num e saldo
					contas.add(new ContaCorrente(nr, saldo)); //adiciona na lista
				}
				if(v[0].equals("1")) { //se na posição 0 listar "1" adiciona uma nova conta poupança adicionando num e saldo
					contas.add(new ContaPoupanca(nr, saldo));
				}
				if(v[0].equals("2")) { //se na posição 0 listar "2" adiciona uma nova conta especial adicionando num, saldo e limite
